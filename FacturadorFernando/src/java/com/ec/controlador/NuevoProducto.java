...
//                this.producto.setProdIva(BigDecimal.ZERO);
            ...
            this.producto.setProdIva(BigDecimal.valueOf(0));
            this.producto.setPordCostoVentaRef(BigDecimal.ZERO);
...
//            calcularPrecioFinalVenta();
        ...
        if (!esUnProdcuto && conIva.equals("S")) {
            producto.setProdIva(BigDecimal.valueOf(15));
...
//        
    ...
    @Command
    @NotifyChange({"muestraSubtotal", "grabaIva","producto","prodPrecioSubtotal"})
...
//            prodPrecioSubtotal
            if (prodPrecioSubtotal.doubleValue() == 0) {
                
                BigDecimal valorIva=producto.getProdIva().divide(BigDecimal.valueOf(100));
                BigDecimal subT = producto.getPordCostoVentaFinal().divide(valorIva.add(BigDecimal.ONE), 4, RoundingMode.CEILING);
                prodPrecioSubtotal = subT;
            } else {
                BigDecimal valorIva=producto.getProdIva().divide(BigDecimal.valueOf(100));
                BigDecimal valorFin=prodPrecioSubtotal.multiply((valorIva.add(BigDecimal.ONE))).setScale(4, RoundingMode.CEILING);
                producto.setPordCostoVentaFinal(valorFin);
...
//        producto.setProdCostoPreferencial(costoPorUtiManTransPref);
    ...
    @Command
    @NotifyChange({"producto","prodPrecioSubtotal"})
    public void calcularPrecioFinalVenta() {

        if (prodPrecioSubtotal != null) {
            if (conIva.equals("S")) {
                BigDecimal valorIva= producto.getProdIva().divide(BigDecimal.valueOf(100));
                BigDecimal precio = prodPrecioSubtotal.multiply(valorIva.add(BigDecimal.ONE));
                this.producto.setPordCostoVentaFinal(precio);
            } else {
...
//            calculopreciofinal();
    ...
    public void calcularSubTotal() {
        ...
        if (this.producto.getPordCostoVentaFinal().doubleValue()>0) {
            if (conIva.equals("S")) {
                BigDecimal valorIva= producto.getProdIva().divide(BigDecimal.valueOf(100));
                BigDecimal precio = this.producto.getPordCostoVentaFinal().divide(valorIva.add(BigDecimal.ONE),4,RoundingMode.CEILING);
               prodPrecioSubtotal=precio;
            } 
        } else {
            Clients.showNotification("Verifique el total",
                    Clients.NOTIFICATION_TYPE_ERROR, null, "middle_center", 2000, true);
        }
